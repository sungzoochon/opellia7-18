...
def enter_position(exchange, coin, cur_price,long_target, coin_amount, position,bought_coin):
   ...
   if position['type'] == 'short': 
    if long_target * 1.01 >= cur_price >= long_target: 
        ...
        type.append(position['type'])  
        amount_list.append(coin_amount * position['leverege'])
        print('[',len(bought_coin),']','\033[31m',"지구 숏",coin,'\033[0m',a * "_",round(usdt2,3))
...
while True: 
  try:
    ...
    if m < len(Market):                                               
        coin = Market[m]
        if (now.hour == 9 and now.minute == 00 and now.second <= 10 and op_mode == True) or once == 1:
        ...
        if start == False and (now.hour == 9 or once == 1):               
            ...
            print(now.year,"/",now.month,"/",now.day,"가즈아")
        if start:
         if long_target[m]  == 0:
          long = 'long'
         ...
         if (usdt2 >= (usdt * 1.7)) or (usdt2 <= (usdt * 0.5)):
            if op_mode:
                print("끝")
                for a in range(len(bought_coin)): 
                    exit_position(binance,a)    
